...
router.get('#/remap/:db/*nodeloc', function (rtr,db,nodeloc){//, db, view) {:db/views/*view
	//console.log(test);//seo urls in couchdb?!
	//console.log(router);
	//console.log(nodeloc);
	//db = "me";
	view = "_tree_docs";
    var dbname = decodeURIComponent(db)
      ...
      , id = null
      , opts = {limit: (view === "_tree_docs" ? 99 : 11)}
      , paginate = (view !== "_tree_docs");
	...
	function buildtree (node, data){
		if(node.doc.parentTo){
			...
			$.each(node.doc.parentTo, function(){
				...
				childnode.path = buildpath(childnode);
	...
	tree = buildtree(data.rows[0], data);
	//console.log(data);

      /*var rows = $.map(data.rows, function(obj) {
        obj.name = encodeURIComponent(obj.id);
        obj.display = JSON.stringify(obj.doc);//JSON.stringify(obj.key, null, ' ') + " <span class='jsonval'>" + JSON.stringify(obj.doc) + "</span>";//obj.value
        return obj;
      });*/

      var parent;
      var rowstmp = $.map(data.rows, function(obj) {
     //  	obj.name = encodeURIComponent(obj.id);
	
        obj.display = JSON.stringify(obj.doc.nodeID);
//	obj.content
	/*JSON.stringify(obj.key, null, ' ') + " <span class='jsonval'>" + JSON.stringify(obj.doc) + "</span>";//obj.value*/
	/*if(nodeloc != "" && obj.doc.parent){
		//console.log(obj);
		if(nodeloc == obj.doc.parent.path)
			parentdoc = obj.doc.parent.doc
			return obj;
	}
	else if(nodeloc == "" && obj.doc.parent){
		if(obj.doc.parent.id == "000000root")
		{
			parentdoc = obj.doc.parent.doc
			return obj;
		}
	...
	if(obj.doc.parent){
		//console.log(obj.doc.parent.path);
		if((nodeloc == obj.doc.parent.path) || (nodeloc == obj.doc.parent.id)){
		//	console.log(obj);
			parent = obj.doc.parent;
			return obj;
		}
		//if(nodeloc == obj.doc.parent.id){
	}
	//if(nodeloc == "000000root" && obj.id == "000000root")
	//	parentdoc = obj.doc;
	if(obj.path == nodeloc)
		parent = obj
	//console.log(parentdoc);
	//ignore 000000root
	/*else if(nodeloc == obj.doc.parent.id){
			parentdoc = obj.doc.parent.doc
			return obj;
	}*/
	...
	if(!parent)
		parent = data.rows[0];
	if(parent.doc.parentTo){
		$.each(parent.doc.parentTo, function(){
			var searchnodeID = this;
			$.each(rowstmp, function(){
				if(this.doc.nodeID == searchnodeID)
					rows.push(this);
			});
		});
	}
	...
	//console.log(rows);
	var dochtmlvalue = "ERROR - Node has no content!";
	var doccontent = "";
	if(parent.content){
		dochtmlvalue = "ERROR - Content has no doc!";
		if(parent.content.doc){
			dochtmlvalue = "ERROR - Doc has no html!";
			if(parent.content.doc.html)
				dochtmlvalue = JSON.stringify(parent.content.doc.html);
				doccontent = parent.content.id;
		}
	}


      renderer.render('remap_tpl', { db: dbname
                                           , hasNext: !end
					...
					, path: nodeloc
					, doc: parent.id
					, docchildren: parent.doc.parentTo
					, key: "parentTo"
					, doccontent: doccontent
					, dochtmlkey: "html"
					, dochtmlvalue: dochtmlvalue
					}, rtr);
    };
    ...
    } else if (view === '_tree_docs') {
      $.couch.db(db).allTreeDocs(opts).then(callback);
    } else {
  ...
  router.post('#create_node', function (_, e, form) {
	var d = new Date();
	var n = d.getTime();
    var nodevalue =  "{ \"nodeID\":\"" + n + "\",\"parentTo\":[],\"content\":\"" + n + "c" + "\"}"
    var obj = parseJSON(nodevalue || "{}");
	...
	var escapedformvalue = form.value;//$('<div/>').text(form.value).html();
console.log(escapedformvalue);
    var contentvalue =  "{ \"contentID\":\"" + n + "c" + "\",\"html\":\"" + escapedformvalue + "\"}"
    var obj2 = parseJSON(contentvalue || "{}");
    if (typeof obj2 !== "object" || $.isArray(obj)) {
      obj2 = {};
    }
	...
	console.log(form);
    $.couch.db(form.db).saveDoc(obj).then(function(json){
		$.couch.db(form.db).saveDoc(obj2).then(function(json) {
     		//location.href = location.href + "?";/*"#/db/" + form.db + "/" + res.id + "/";*/
    		
			    $.couch.db(form.db).openDoc(form.doc).then(function(json) {
			      for(var tmp = json, keys = form.key.split(":"), i = 0; i < keys.length-1; i++) {
				tmp = tmp[keys[i]];
			      }
				var docchildrenarr = parseJSON("["+form.docchildren+"]");
			      docchildrenarr.push( n );
			      tmp[keys[keys.length-1]] = docchildrenarr;//parseJSON(form.value);
			      $.couch.db(form.db).saveDoc(json).then(function(json) {
				router.refresh();//location.href = location.href + "?";
			      });
			    });
  ...
  router.post('#savekey', function (_, e, form) {
    $('#savekey').val('Saving ...');
    $.couch.db(form.db).openDoc(form.doc).then(function(json){	console.log(json);}).then(function(json) {
